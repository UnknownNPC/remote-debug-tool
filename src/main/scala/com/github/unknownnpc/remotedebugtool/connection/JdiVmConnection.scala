...
case class JdiVmConnection(address: Address, port: Port) extends VmConnection {
  ...
  private var breakpointRequest: Option[BreakpointRequest] = None
  ...
  override def enableBreakpoint(line: BreakpointLine, className: BreakpointClassName) {
    ...
    breakpointRequest = Option(createBreakpointBy(location))
    breakpointRequest.getBreakpointReq.enable()
  }

  override def disableBreakpoint() {
    breakpointRequest.getBreakpointReq.disable()
  }
  ...
  private def searchInEventSet(fieldName: FieldName,
                               triggeredEventSet: EventSet): List[Option[BreakpointValue]] = {
